...
| 7.3 ✅ | **Blocking** | **End-to-end context test in CI.** The full Spring context had never booted before v1 — two startup bugs (MCP bean cycle, missing `CarbonAwareZoneSelector` bean) only surfaced on first container run. **Done:** added `ContextLoadsTest` (`@SpringBootTest @Tag("integration")`, makes no provider call, asserts the graph wires) and a GitHub Actions CI (`.github/workflows/ci.yml`): a `build` job (`./mvnw -DskipFrontend verify`) + an `integration` job that runs `./mvnw -Pit test` against Postgres + Ollama service containers (Claude-calling smoke test auto-skipped — no key). Wiring regressions now fail the build. |
| 7.4 ✅ | Hardening | **Cheap/offline testing.** Free path via Ollama is already on (7.2). **Done:** added a `mock` Spring profile with `MockEchoChatModel` (`@Primary`, deterministic echo egress at the `ChatModel` level, so cache + routing + green accounting still run); `DelegatingChatModel` is `@Profile("!mock")`. Run with `SPRING_PROFILES_ACTIVE=mock` → full app, dashboard and reports work with **no API key and zero cost**. Verified in-container (200, echoed reply, green report populated). |
| 7.5 ✅ | Hardening | **Streaming (full).** `stream: true` now returns SSE (`chat.completion.chunk` deltas + `[DONE]`). **Done:** callback-based stream port (`Consumer<LlmStreamChunk>` — keeps Reactor out of the domain), `SpringAiLlmClient.stream` bridges `Flux→Consumer` via `toStream()`, `RoutingAdvisor`/`SemanticCacheAdvisor` `adviseStream` implemented (router reroutes; cache: **synthetic Flux on hit**, aggregate-and-store on miss), green accounting on stream completion, and the controller emits SSE on a virtual thread with the `RequestContext` Scoped Value re-bound. Verified in-container: real token streaming via Ollama (free), cache-hit synthetic replay, accounting populated. |
| 7.6 ~ | Hardening | **Carbon calibration.** ~~Replace the placeholder per-model `energyIntensity` coefficients with measured values~~ — **half delivered by v3 lot C**: the scalar is gone (prefill/decode split, every coefficient sourced with a URL and a read-date, PUE per provider instance, per-row provenance), but nothing is *measured*: self-hosted egress is **excluded from scope** and cloud figures stay modelled or vendor-published ([ADR 0013](../technical/adr/0013-sourced-and-labelled-not-measured.md)). Metering local inference is a later lot D. Marginal grid intensity (WattTime) is still open — see [`carbon-intensity-reliability.md`](../technical/carbon-intensity-reliability.md) §5. |
| 7.7 ✅ | Hardening | **Cluster-readiness.** **Done by v3 lot B**, without Redis or Hazelcast: the deferred-job store, the rate-limit buckets and the live routing config all moved into the Postgres already required (`FOR UPDATE SKIP LOCKED` claims, `pg_try_advisory_xact_lock` leader gating), and Flyway replaced `ddl-auto=update` in Phase 6. Proven by `scripts/cluster-smoke.sh` on two replicas — see [`../technical/clustering.md`](../technical/clustering.md). |
