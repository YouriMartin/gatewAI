...
- [~] v3 (`docs/developpment/roadmap-v3.md`) — **lots A and B done, lot C planned**; **lot A**: in-process ONNX embedding (`paraphrase-multilingual-MiniLM-L12-v2`, int8, 384d, fetched at build time; PyTorch engine excluded; jar 161→349 MiB; boots in 7.9 s with no model server), vector schema 768→384 (upgrade = `DROP TABLE vector_store`; skipping it silently kills the cache, traced `outcome=ERROR`), model chosen on measurements (82.0 % calibrated routing vs 73.0 % EN-only and 81.0 % e5), fixtures/baselines/calibrations redone (`route-similarity-threshold` 0.60→**0.25**, q̂ 0.9526 cache / 0.2221 routing, decisions 34→**3.2 ms** p50), native hints + [ADR 0011](docs/technical/adr/0011-in-process-onnx-embedding.md).
  ...
  - **lot C — cloud carbon** (planned): make the CO2 figure for **cloud** egress sourced and region-attributed. Fixes a real defect first: `ChatCompletionService.accountGreen` resolves **one** grid intensity for every request (the gateway's own zone), so a `zone=FR` gateway books a Claude call — US compute, plausibly 350–400 gCO2/kWh — at France's 56. Region is absent, not approximate.
    - [ ] C.1 — local egress declared **out of scope** and booked at zero (`energy-intensity=0`), with an `EnergySource` enum (`NOT_ACCOUNTED` | `VENDOR_PUBLISHED` | `MODELLED`) so every renderer prints "excluded from scope", never a bare `0 gCO2`. Metering local inference is lot D
    - [ ] C.2 — `region` + `region-provenance` (`KNOWN` | `ASSUMED`) + `pue` on the **provider instance**, not the model; cloud-region → grid-zone mapping, config-overridable, warn-and-fall-back on unknown
    - [ ] C.3 — resolve intensity per request: dispatch zone (operator-controlled providers only) → provider region → gateway default. For a hosted API the dispatch zone is **recorded, not applied** — deferring a job does not move Anthropic's compute
    - [ ] C.4 — prefill/decode split (`a×promptTokens + b×completionTokens + c`; both already persisted) and sourced coefficients: vendor-published where it exists, EcoLogits/Boavizta parametric otherwise, each with source + read-date
    - [ ] C.5 — `grid_zone`, `grid_intensity_g_per_kwh`, `energy_source` on `RequestLog` (`V9`) so a row re-derives its own number; reporting broken down by region and provider
    - [ ] C.6 — ADR 0012 (region on the provider instance) + ADR 0013 (sourced-and-labelled over measured); doc honesty pass
